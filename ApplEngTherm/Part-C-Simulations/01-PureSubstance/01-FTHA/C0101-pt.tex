...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Parâmetros do mecanismo}\vspace*{-2em}
        ...
        \column{0.50\textwidth}
        \begin{itemize}
            \item<1-> \alert{Diâmetro} do pistão/cilindro, $D$;
            \item<2-> \alert{Raio} da manivela, $R$;
            \item<3-> \alert{Curso} do pistão, $S = 2R$;
            \item<4-> \alert{Comprimento} da biela, $L$;
            \item<5-> \alert{Volume} morto (do PMS), $V_{\mathrm{min}}$;
            \item<6-> \alert{Posição} do pistão (rel.~PMS), $x$;
        \end{itemize}
        \column{0.50\textwidth}
        \begin{center}
            \uncover<1->{\includegraphics[height=60.0mm]{fig/FTHA_Engine.pdf}}
        \end{center}
        \end{columns}
    \end{frame}
    %-------------------------------------------------------------------------------------------
