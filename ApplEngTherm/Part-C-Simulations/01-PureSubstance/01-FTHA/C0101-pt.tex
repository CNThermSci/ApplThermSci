...
%-----------------------------------------------------------------------------------------------
\subsection{Parte~I: Apresentação do Modelo FTHA}
%-----------------------------------------------------------------------------------------------
...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Algoritmo de Inicialização}\vspace*{-2em}
        \small
        \mode<beamer>{\begin{tcolorbox}[colback=Tcya!80!white]}
        \mode<handout>{\begin{tcolorbox}[colback=Tcya!20!white]}
        \begin{algorithmic}[1]
    ...
    \begin{frame}{Algoritmo de Finalização}\vspace*{-2em}
        ...
        \begin{algorithmic}[1]
            \STATE{$w_{ent} \leftarrow \sum w_i \geqslant 0$;}
                \COMMENT{Trabalho que entra no sistema em um ciclo}
            \STATE{$w_{out} \leftarrow - \sum w_i < 0$;}
                \COMMENT{Trabalho realizado pelo sistema em um ciclo}
            \STATE{$w_{net} \leftarrow w_{out} - w_{ent}$;}
                \COMMENT{Trabalho líquido realizado pelo sistema no ciclo}
            \STATE{$q_{ent} \leftarrow \sum q_i \geqslant 0$;}
                \COMMENT{Calor que entra no sistema em um ciclo}
            \STATE{$q_{rej} \leftarrow - \sum q_i < 0$;}
                \COMMENT{Calor rejeitado pelo sistema em um ciclo}
            \STATE{$\eta_t  \leftarrow w_{net} / q_{ent}$;}
                \COMMENT{Eficiência térmica}
            \STATE{$r_{bw}  \leftarrow w_{ent} / w_{out}$;}
                \COMMENT{Razão de consumo de trabalho}
    ...
    \begin{frame}{Algoritmo de Solução de Sub-Processo}\vspace*{-2em}
        ...
        \begin{algorithmic}[1]
            \IF{$|v_i - v_{i+1}| \leqslant \epsilon_v$}
                ...
                \STATE{Calcula $n_i$ pelo processo politrópico ou faz $n_i \leftarrow +\infty$ em caso de excessão;}
            \ELSE
    ...
    \begin{frame}{Algoritmo de Solução de Sub-Processo Politrópico}\vspace*{-2em}
        ...
        \begin{algorithmic}[1]
            ...
            \STATE{$n_i$, $w_i$, $u_{i+1}$, $T_{i+1}$ e $P_{i+1}$ $\leftarrow$ seus últimos
                elementos $j$;} \COMMENT{Reverte vetores (linha 2)}
        \end{algorithmic}\end{tcolorbox}
    \end{frame}
    %-------------------------------------------------------------------------------------------
...
%===============================================================================================
\part{Validação e Estudo de Caso com FTHA}
\frame{\partpage}
%===============================================================================================

%-----------------------------------------------------------------------------------------------
\section{Validação do Modelo FTHA}
%-----------------------------------------------------------------------------------------------

    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{Importância da Validação}\vspace*{-2em}
        \begin{itemize}
            ...
            \item<9-> Como saber se a solução numérica não retorna ``\textit{garbage\/}''?
                \uncover<10->{$\rightharpoondown$ \alert{Validação}!}
        \end{itemize}
    \end{frame}
    %-------------------------------------------------------------------------------------------
    ...
    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{O que é Validação?}\vspace*{-2em}
        \uncover<1->{Resultados de um modelo numérico só são \alert{confiáveis} se o modelo for
            \alert{validado}:}
        \begin{itemize}
...
%-----------------------------------------------------------------------------------------------
    ...
    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{Recapitulando: Desvios do ciclo Otto ideal}\vspace*{-2em}
        \begin{center}
            \includegraphics[height=40mm,width=60mm]{%
                fig/P-V_diagram_deviations_to_Otto_cycle.pdf}\\
    ...
    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{Estudo de Caso}\vspace*{-2em}
        \begin{columns}
        \column{0.60\textwidth}
        \begin{itemize}
            ...
            \item<2-> \alert{$V_{du} = \unit{250}{\centi\meter\cubed}$}, $L/R = 5$ e \alert{$r =
                12$:$1$};
            \item<3-> Fluido de trabalho \alert{\ce{CO2}} como \alert{gás ideal} e
...
%-----------------------------------------------------------------------------------------------
    ...
    % Finishes with stunning image, with credit
    \ArtEndW{pexels-paul-basel-1816842}{jpg}{txt}
