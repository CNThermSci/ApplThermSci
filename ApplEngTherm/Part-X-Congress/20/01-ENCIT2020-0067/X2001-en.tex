...
%-----------------------------------------------------------------------------------------------
    ...
    %The model can thus serve as a teaching resource in advanced engineering classes, as well  as
    %a departure point for more complete equilibrium models that account for:  valves  and  flow,
    %combustion and chemical equilibrium, with heat-transfer and pressure-drop irreversibilities,
    %on the thermodynamic ``side'', as well as progressively more detailed dynamic elements, such
    %as more complex mechanisms  and  unbalanced  elements,  coupling  and  decoupling  clutches,
    %multiple gears, multi-cylinder engine, friction irreversibilities, and the like.

        \begin{itemize}
            \item Results show model able to predict \alert{$\mathsf{T}(t)$},
                \alert{$\omega(t)$}, \alert{$P$-$v$} behavior, \alert{$\eta_t$}, and
                \alert{$\bar{\mathsf{T}}(\omega_0)$};
            \item Many \alert{energy transfer modes} are accounted for;
            \item Model with reasonable amount of design/application \alert{input parameters};
            \item Model can thus serve as a \alert{teaching resource} in advanced engineering
                classes;
            \item Or as a departure point for more complex equilibrium models in
                \alert{research}:
        \end{itemize}
    \end{frame}
    %-------------------------------------------------------------------------------------------
