...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Correção do Expoente Politrópico}\vspace*{-2em}
        \begin{itemize}
            \item<1-> Com $n_i^j$ é possível obter $w_i^j$ e $u_{i+1}^j$ por balanço de energia;
            \item<2-> $P_{i+1}$ pode ser obtida via $u_{i+1}^j$ e modelo de substância;
            \item<3-> O novo expoente $n_i^{j+1}$ pode ser achado pelo processo politrópico:
        \end{itemize}
        \begin{equation}
            \uncover<4->{\alert{
                n_i^{j+1} = \dfrac{
