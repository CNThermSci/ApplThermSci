...
%-----------------------------------------------------------------------------------------------
    ...
    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{Primeira Lei da Termodinâmica}\vspace*{-2em}
        \uncover<1->{A \alert{1$^{\mathrm{a}}$ lei da termodinâmica} estabelece que:}
        \begin{itemize}
            \item<1-> \alert{Energia} é uma quantidade \alert{conservada}. \\[\medskipamount]
        \end{itemize}
        \uncover<2->{Este \alert{princípio da conservação da energia}:}
        \begin{itemize}
            \item<2-> é exaustivamente \alert{confirmado} em experimentos. \\[\medskipamount]
        \end{itemize}
        \uncover<3->{Logo, no universo observável:}
        \begin{itemize}
            \item<3-> \alert{Não há processos físicos} que \alert{criem} energia,
                \\[\medskipamount]
            \item<4-> \alert{nem processos físicos} que \alert{destruam} energia.
                \\[\medskipamount]
            \item<5-> \alert{Processos físicos} podem apenas \alert{converter} energia de uma
                forma a outra. \\[\medskipamount]
        \end{itemize}
        \uncover<6->{A relatividade de Einstein \alert{unificou} as conservações de
            \alert{massa} e de \alert{energia} via $E = c^2m$.
        }
    \end{frame}
    %-------------------------------------------------------------------------------------------
