...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Fração Residual:}\vspace*{-2em}
        %-----
        \begin{itemize}
            \item<1-> Uma \Alert{fração residual, $\zeta$}, de gases do ciclo anterior permanece
                no sistema (volume morto $>0$);
                \\[\bigskipamount]
            \item<2-> Silva, R.~K.~de O.~modelou dados de Heywood, J.~B., resultando em:
        \end{itemize}
        \vspace*{0.8em}\begin{align*}
            \uncover<3->{
                \alert{\zeta(P, r)}
                    &\alert{= 17.807 + 6.423\mathtt{g}(r) - [0.029 + 0.013\mathtt{g}(r)] P} \\
