...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Parâmetros de tempo do motor}\vspace*{-2em}
        ...
        \begin{itemize}
            ...
            \item<4-> ``\alert{Duração angular}'' da combustão, \alert{$\delta = \omega\Delta t_c$};
                \\[\medskipamount]
            \item<5-> Casos de $\omega$ constante---discretização em $\alpha$:
            \begin{itemize}
            ...
            \end{itemize}
            \item<7-> Casos de $\omega$ variável---discretização em $t$.
        \end{itemize}
        \column{0.40\textwidth}
        \begin{center}
    ...
    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{Modelo de Fração Cumulativa de Adição de Calor, $y$}\vspace*{-2em}
        \begin{columns}
        \column{0.50\textwidth}
        \begin{align}
            y(\alpha)   &=
        ...
        \end{align}
        \begin{itemize}
            \item<2-> \alert{$g(\alpha)$} modela o \alert{histórico} da lib.~de calor:
            \begin{itemize}
                \item<3-> \alert{$g(\theta) = 0$} e \alert{$g(\theta+\delta) = 1$};
                \item<4-> \alert{Função} $g(\alpha)$ deve ser \alert{monotônica};
                \item<5-> $g(\alpha)$ pode basear-se em \alert{experimentos};
                \item<6-> Lit.:~$g(\alpha) = \frac{1}{2}-\frac{1}{2} \cos( \frac{\pi}{\delta}
                    (\alpha - \theta) )$.
            \end{itemize}
        \end{itemize}
        \column{0.50\textwidth}
        \begin{center}
            \uncover<1->{\vspace*{-5mm}\includegraphics[height=70.0mm]{fig/FTHATiming01.pdf}}
        \end{center}
        \end{columns}
    \end{frame}
    %-------------------------------------------------------------------------------------------
