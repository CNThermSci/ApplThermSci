...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Equações Termodinâmicas}\vspace*{-2em}
        \uncover<1->{(Sub-)Processo politrópico $i | i \in \mathbb{Z}$:}
        \begin{itemize}
            \item<2-> Sistema evolui do \alert{estado-$i$} para o \alert{estado-$(i+1)$}.
            \item<3-> \alert{Estados} com \alert{propriedades} $P_i$, $T_i$, $v_i$, $u_i$, etc.
            \item<4-> \alert{Interações} do \alert{processo} são $q_i$ e $w_i$.
                \\[\bigskipamount]
        \end{itemize}
