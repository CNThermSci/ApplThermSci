...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}{Aplicações}\vspace*{-2em}
        \uncover<1->{A 1$^{\mathrm{a}}$ lei é \alert{central} em termodinâmica. Suas aplicações
        são \alert{vastas} e incluem:}
        \begin{itemize}
            \item<2-> Princípio em variedade de \alert{deduções}; \\[\medskipamount]
            \item<3-> Instrumental na definição de \alert{propriedades}. \\[\medskipamount]
            \item<4-> Cálculos de \alert{processos} energéticos. \\[\medskipamount]
        \end{itemize}
        \uncover<5->{Exemplo: O que é, afinal, ``energia''?}
        \begin{itemize}
            \item<6-> ``Energia é uma \alert{quantidade} (escalar) \\[\medskipamount]
            \item<7-> que é \alert{conservada} na natureza \\[\medskipamount]
            \item<8-> e que possui \alert{unidades de
                \kilogram\usk\squaren\meter\per\squaren\second}.'' %\\[\medskipamount]
        \end{itemize}
        \uncover<8->{\hfill --- Jack P.~Holmann (SMU)}
    \end{frame}
    %-------------------------------------------------------------------------------------------
...
%-----------------------------------------------------------------------------------------------
    ...
    % !j 96 -i8
    %-------------------------------------------------------------------------------------------
    \begin{frame}{Matematicamente}\vspace*{-2em}
        \uncover<3->{A 1$^{\mathrm{a}}$ lei é matematicamente expressa:}
        \begin{itemize}
            \item<3-> Por meio de \alert{balanço de energia}.
        \end{itemize}
        \uncover<1->{Logo, no universo observável:}
        \begin{itemize}
            \item<1-> \alert{Não há processos físicos} que \alert{criem} energia,
