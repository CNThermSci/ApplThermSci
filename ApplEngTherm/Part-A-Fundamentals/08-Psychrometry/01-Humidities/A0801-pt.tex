...
%-----------------------------------------------------------------------------------------------
    ...
    \begin{frame}[allowframebreaks]{Ar Atmosférico --- Em Grupos}\vspace*{-0em}
        ...
        \alert{$\unit{50}{\kilo\meter}$ de altitude}.\\[\medskipamount]

        \begin{itemize}
            \item A \alert{Troposfera} vai do nível do mar \alert{até $\unit{8\mbox{ a }15}%
                {\kilo\meter}$};
            \item A \alert{Estratosfera} vai da Troposfera \alert{até $\unit{50\mbox{ a }60}%
                {\kilo\meter}$};
            \item A composição \alert{majoritária} seguirá a \alert{Tabela~25} da ref.~abaixo:
            \item {\scriptsize\tt
                https://ntrs.nasa.gov/api/citations/19770009539/downloads/19770009539.pdf}
            \item A parcela de \alert{Ozônio (\ce{O3})} será agregada de outra fonte;
            \item A soma das frações deverá ser normalizada para $\unit{100}{\%}$;
            \item Gerar um notebook aberto ({\tt CC-BY-NC}) que produza as ilustrações.
        \end{itemize}
        ...
        \begin{columns}
            ...
            \begin{itemize}
                ...
                \item A parcela de \alert{Ozônio (\ce{O3})} será obtida por integração:
                \item A integração numérica deverá ser documentada em notebook aberto;
                \item Utilizar método de Simpson 3/8 composto ou melhor.
            \end{itemize}
            \column{0.4\textwidth}
            \wwwfig{1-01}{jpg}{url}
        \end{columns}
        ...
        \begin{columns}
            \column{0.7\textwidth}
            \begin{itemize}
                \item Ref.~programação de \alert{imagens} e \alert{notebooks} no curso:
                \item {\scriptsize\tt https://computationalthinking.mit.edu/Fall20};
                \item Inclui \alert{instalações} ({\tt Julia, Pluto}).
                \item Notebook deve gerar ilustrações com gases \alert{agrupados} e
                    \alert{dispersos}, e permitir \alert{seleção de camadas} da atmosfera (km
                    inteiros).
            \end{itemize}
            \column{0.3\textwidth}
            \wwwfig{julia-logo}{pdf}{url}
        \end{columns}
